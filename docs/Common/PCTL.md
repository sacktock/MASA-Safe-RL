...
The formula holds in state ``s`` if the probability that $\Phi$ holds in the
...
The formula holds in state ``s`` if the probability that $\Phi_2$ becomes true
within ``B`` steps while $\Phi_1$ holds at all preceding steps is at least ``p``.
...
where $P_k(s)$ is the probability that the subformula's path condition is
satisfied within $k$ steps starting from state $s$.
...
expectations of the form $\mathbb{E}[V(s')]$ by either:
...
- [Propositional Operators](PCTL/Propositional%20Operators.md) - API Reference for PCTL propositional operators.
- [Temporal Operators](PCTL/Temporal%20Operators.md) - API Reference for PCTL temporal operators.
- [Model Checking](PCTL/Model%20Checking.md) - API Reference for PCTL model checking.
