...
Below is a **concise README** you can drop into your repository (or adapt slightly). It is written to match the structure and usage patterns of your implementation and examples.

Overview
--------

Probabilistic Shielding addresses RL problems of the form:
...
3. **Restrict actions via a shield**
   At each augmented state ((s, q)), only actions that **provably preserve the safety bound** are allowed.
   This is enforced by projecting agent-selected actions onto a safe probability simplex.
...
The main entry point is the Gym wrapper:
